				...
				runs: {
					build: (
						...
						+ (
							.tags
							| map(
								["--tag " + ("littleof/" + . | @sh), "--tag " + ("ghcr.io/littlezo/" + . | @sh), "--tag " + ("registry.cn-beijing.aliyuncs.com/forlong/" + . | @sh)] | join(" ")
							)
						)
					...
					push: ((
							.tags
							| map(
								["docker push " + ("littleof/" + . | @sh), "docker push " + ("ghcr.io/littlezo/" + . | @sh), "docker push " + ("registry.cn-beijing.aliyuncs.com/forlong/" + . | @sh)] | join(" && ")
							)
						) | join(" && ")),
					history: ("docker history " + ("ghcr.io/littlezo/" + .tags[0] | @sh)),
					test: (
						[
							"set -- " + ("ghcr.io/littlezo/" + .tags[0] | @sh),
